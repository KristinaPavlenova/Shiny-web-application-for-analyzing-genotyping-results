...
dir_for_blast_results = 'blast_results_COI' # указать директорию для результатов blast-выравнивания
dir_for_mafft_results = 'mafft_results_COI' # указать директорию для результатов mafft-выравнивания
log_file = os.path.join('logs', 'main_script_errors.log') # указать файл для логов
reference = os.path.join('references', 'Eulimnogammarus_verrucosus_COI.fasta') # указать fasta-файл с референсом
...
idxs = list(range(len(fastanames))) #указать индексы (номер по порядку) входных fasta-файлов для пайплайна
...
for fasta in fastanames:
    file = os.path.join("input_fasta", fasta)
    ...
    subprocess.run(['makeblastdb', 
                    ...
                    '-title', file,
                    '-dbtype', 'nucl',
                    '-out', os.path.join('databases', fasta)])
    subprocess.run(['blastn',
                      '-db', 'databases/' + fasta,
                      '-query', reference,
                      '-evalue', '1e-3',
                      '-word_size', '11',
                      '-outfmt', '6 sseqid evalue slen',
                      '-out', os.path.join(dir_for_blast_results, fasta)])
    ...
    with open(blast_top_hits, 'w') as out_file:
        ...
        if assembly == 'rnaspades':
            if gene == 'COI':
                awk_extract = subprocess.Popen(["awk", r'''{match($1, /length_([0-9]+)/, len); match($1, /cov_([0-9.]+)/, cov);
                                                if (len[1] >= 1400 && cov[1] >= 10) {print $0 "\t" cov[1];}}''',
                                                os.path.join(dir_for_blast_results, fasta)], stdout=subprocess.PIPE)
            else:
                awk_extract = subprocess.Popen(["awk", r'''{match($1, /length_([0-9]+)/, len); match($1, /cov_([0-9.]+)/, cov);
                                                if (len[1] >= 2000 && cov[1] >= 10) {print $0 "\t" cov[1];}}''',
                                                os.path.join(dir_for_blast_results, fasta)], stdout=subprocess.PIPE)
            sort_e = subprocess.Popen(['sort', '-k2,2g', '-k4,4gr', '-s'], stdin=awk_extract.stdout, stdout=subprocess.PIPE)
        else:
            awk_extract = subprocess.Popen(["awk", f"$3 >= {len_filter}",
                                            os.path.join(dir_for_blast_results, fasta)], stdout=subprocess.PIPE)
            sort_e = subprocess.Popen(['sort', '-k2,2g', '-s'], stdin=awk_extract.stdout, stdout=subprocess.PIPE)
        cut_id = subprocess.Popen(['cut', '-f', '1'], stdin=sort_e.stdout, stdout=subprocess.PIPE)
    ...
    try:
        ...
        with open(mafft_result, "w") as out_file:
            subprocess.run(["mafft", "--maxiterate", "1000", "--globalpair", "--adjustdirection", top_ref], stdout=out_file)
    ...
    try:
        ...
        with open(best_to_ref, 'w') as out_file:
            ...
            for record in alignment:
                matches = sum(1 for a, b in zip(reference_sequence.seq, record.seq) if a == b and a != '-')
            ...
            if assembly == 'rnaspades':
                df = pd.DataFrame(results, columns=["Sequence ID", "Avg Identity (%)", "Coverage"])
                df_sorted = df.sort_values(by=["Avg Identity (%)", "Coverage"], ascending=False, ignore_index=True)
            else:
                df = pd.DataFrame(results, columns=["Sequence ID", "Avg Identity (%)"])
                df_sorted = df.sort_values(by=["Avg Identity (%)"], ascending=False, ignore_index=True)
    ...
    try:
        ...
        with open(trimmed_best, 'w') as out_file:
            ...
            start = next((i for i, char in enumerate(reference_sequence) if char != '-'), -1)
            end = len(reference_sequence) - next((i for i, char in enumerate(reference_sequence[::-1]) if char != '-'), -1) - 1
            trimmed_seq = alignment[-1].seq[start:end + 1]
            trimmed_ref = reference_sequence[start:end + 1]
            cleaned_seq = ''.join(base for base, ref_base in zip(trimmed_seq, trimmed_ref) if not (base == '-' and ref_base == '-'))
            trimmed_record = SeqRecord(Seq(cleaned_seq), id=alignment[-1].id, description="")
