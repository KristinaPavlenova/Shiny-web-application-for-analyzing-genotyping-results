...
def align_and_score(seq1, seq2, aligner):
    alignment = aligner.align(seq1, seq2)[0]
    aln_seq1 = alignment.aligned[0]
    aln_seq2 = alignment.aligned[1]
    matches = 0
    for (start1, end1), (start2, end2) in zip(aln_seq1, aln_seq2):
        matches += sum(seq1[start1:end1][i] == seq2[start2:end2][i] for i in range(min(end1-start1, end2-start2)))
...
def align_wo_gaps(seq1, seq2, aligner):
...
def save_alignment_text(alignment, output_path):
    alignment_str = alignment.format()
    with open(output_path, "w") as f:
        f.write(alignment_str)


def collect_trimmed_seqs(dir, assembly, gene, out_dir, cleaned=False):
    fastanames = glob.glob("*_trimmed_best.fasta", root_dir=dir)
    result_file = os.path.join(out_dir, f'{assembly}_{gene}_{'cleaned' if cleaned else ''}.fasta')
