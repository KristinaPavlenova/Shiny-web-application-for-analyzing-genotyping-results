...
logging.basicConfig(filename='blast_request_18S.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

os.makedirs('top1_check', exist_ok=True)
table_path = 'top1_check/blast_summary_18S.txt'
...
def extract_hits(blast_text):
    ...
    for entry in entries:
        ...
        # identity_match = re.search(r'Identities\s*=\s*\d+/\d+\s*\(([\d.]+)%\)', entry)
        identities_match = re.search(r'Identities\s*=\s*(\d+)\s*/\s*(\d+)', entry)
        align_len_match = re.search(r'Identities = *(\d+)/(\d+)', entry)
        evalue_match = re.search(r'Expect *= *([\deE.-]+)', entry)

        if identities_match and align_len_match and evalue_match:
            # identity = float(identity_match.group(1))
            # align_len = int(align_len_match.group(2))
...
for fasta_file in fasta_files:
    ...
    with open(fasta_file, "r") as f:
        ...
        if len(clean_seq) < 2200 or len(clean_seq) > 2300:
